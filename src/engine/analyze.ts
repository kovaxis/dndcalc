...
function isDefined(parsed: Parsed, name: string): boolean {
  return (
    name in parsed.defs ||
    parsed.definedParams.find((group) =>
      group.params.find((param) => param.id === name)
    ) != null
  );
}
...
function parseAttribs<T>(raw: string, model: AttribModel): T {
  ...
  return Object.fromEntries(
    Object.entries(model).map(([key, validate]) => {
...
function parseLine(line: string, into: Parsed, lineNum: number) {
  ...
  const isParam =
    /^\s*parameter(\s*(?:\[[^\]]*\])+\s*|\s+)([a-zA-Z_][a-zA-Z_0-9]*)\s*\?\s*([^=]*?)\s*(?:=\s*([0-9]*)\s*)?$/.exec(
      line
    );
  let name = null;
  try {
    if (isSpell) {
      ...
      name = isGroup[2];
      const attribs = parseAttribs<GroupAttribs>(
        isGroup[1] ?? "",
        GROUP_ATTRIBS
      );
      ...
      name = isParam[2];
      const attribs = parseAttribs<ParamAttribs>(
        isParam[1] ?? "",
        PARAM_ATTRIBS
      );
...
export function analyzeSpell(ctx: Context, spell: ParsedSpell): SpellAnalysis {
  ...
  const damage: Map<number, number> = new Map(
    result.bins.entries().map(([val, cnt]) => {
  ...
  try {
    ...
    for (const ref of getReferences(expr)) {
      if (ref in parsed.defs) {
        if (seen.get(ref) === "wip")
          throw `Circular dependency between ${name} and ${ref}`;
...
export function analyze(
  ...
  const parsed: Parsed = {
    definedParams: [
      { name: "", attribs: parseAttribs("", GROUP_ATTRIBS), params: [] },
    ],
    defs: {},
    spells: [],
  };
  ...
  analysis.spells.sort(
    cmpKeyed((analysis) => [
      typeof analysis.average === "number" ? -analysis.average : Infinity,
      analysis.name,
      analysis.lineNum,
    ])
  );
