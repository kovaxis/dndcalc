...
>
  ...
  {#if props.detail}
    {#each ranges as [l, r]}
      {@const prob = table.slice(l, r).reduce((prev, prob) => prev + prob, 0)}
      {@const avg =
        table.slice(l, r).reduce((acc, prob, val) => acc + val * prob, 0) /
          (prob || 1) +
        l}
      {@const z = prob / (r - l) / maxProb}
      {@const xTextIdeal = (w * (avg + 0.5)) / n}
      {@const xText =
        xTextIdeal < fontSize ? 0 : xTextIdeal > w - fontSize ? w : xTextIdeal}
      {@const xTextAlign =
        xTextIdeal < fontSize
          ? "start"
